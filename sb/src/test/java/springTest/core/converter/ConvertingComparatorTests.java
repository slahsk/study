...
public class ConvertingComparatorTests {
	...
	private final StringToInteger converter = new StringToInteger(); //string -> integer 변경

	private final ConversionService conversionService = new DefaultConversionService();

	private final TestComparator comparator = new TestComparator(); //비교 
	...
	private void testConversion(ConvertingComparator<String, Integer> convertingComparator) {
		assertThat(convertingComparator.compare("0", "0"), is(0));	// == 0
		assertThat(convertingComparator.compare("0", "1"), is(-1)); // < -1
		assertThat(convertingComparator.compare("1", "0"), is(1));	// > 1
	...
	public void shouldGetMapEntryKeys() throws Exception {
		...
		assertThat(list.get(3).getKey(), is("d"));
	}
	...
	public void shouldGetMapEntryValues() throws Exception {
		...
		assertThat(list.get(3).getValue(), is(10));
	}
