...
class _tree_struct {
	...
	function _get_node($id) {
		syslog(LOG_NOTICE, "SELECT `".implode("` , `", $this->fields)."` FROM `".$this->table."` WHERE `".$this->fields["id"]."` = ".(int) $id);
	...
	function _get_children($id, $recursive = false) {
		...
		//SELECT `id` , `parent_id` , `position` , `left` , `right` , `level` , `title` , `type` FROM `tree` WHERE `parent_id` = 1 ORDER BY `position` ASC
		//SELECT `id` , `parent_id` , `position` , `left` , `right` , `level` , `title` , `type` FROM `tree` WHERE `parent_id` = 2 ORDER BY `position` ASC
...
class json_tree extends _tree_struct { 
	...
	function get_children($data) {
		...
		if((int)$data["id"] === 1 && count($tmp) === 0) {	//루트면
			$tmp = $this->_get_children((int)$data["id"]);
		}
